...
Omegaini <- modelSelection:::initialEstimateGGM(covid1_scaled, Omegaini='glasso-bic')
...
GLASSO_BIC_sim_results <- foreach(j=1:N, .combine = 'cbind', .packages = c("modelSelection", "matrixStats")) %dopar% {
...
GLASSO_EBIC_sim_results <- foreach(j=1:N, .combine = 'cbind', .packages = c("modelSelection", "matrixStats")) %dopar% {
...
Gibbs_sim_results <- foreach(j=1:N, .combine = 'cbind', .packages = c("modelSelection", "matrixStats")) %dopar% {
...
GIMH_sim_results <- foreach(j=1:N, .combine = 'cbind', .packages = c("modelSelection", "matrixStats")) %dopar% {
...
bdgraph.mpl_sim_results <- foreach(j=1:N, .combine = 'cbind', .packages = c("modelSelection", "mvtnorm", "BDgraph", "matrixStats")) %dopar% {
...
regression.pl_sim_results <- foreach(j=1:N, .combine = 'cbind', .packages = c("modelSelection", "mvtnorm", "BDgraph", "matrixStats", "Matrix")) %dopar% {
