...
Omegaini <- modelSelection:::initialEstimateGGM(covid1_scaled, Omegaini='glasso-ebic')
...
## Mixing 
traceplot.fit.Gibbs <- traceplot.modelSelection(fit.Gibbs, p)

plotcoda.fit.Gibbs <- plotcoda.modelSelection(fit.Gibbs, p)
...
fit.Gibbs_traceplot <- traceplot_llp.modelSelection(y = covid1_scaled, modelSelection.object = fit.Gibbs, p, thinning = 100)
...
traceplot.fit.GIMH <- traceplot.modelSelection(fit.GIMH, p)
    
plotcoda.fit.GIMH <- plotcoda.modelSelection(fit.GIMH, p)
...
iter_multiplier <- 2*ceiling(sqrt(p))
...
BDgraph_traceplot(fit.bdgraph.mpl, thin = iter_multiplier, main = "bdgraph.mpl: Trace of graph size")

BDgraph_plotcoda(fit.bdgraph.mpl, verbose = FALSE, thin = iter_multiplier, thin_plot = 100, main = "Cumulative posterior edge probability", ylab = "Posterior edge probability")
...
fit.regression.pl <- regression.pl_GGM(Y=covid1_scaled, iter=(iterations+warmup), burnin = warmup, w_slab=prior_params_covid$w_slab, s_slab=prior_params_covid$s_slab, lambda=prior_params_covid$lambda, Omega_init=Omega_init, store_samples = FALSE, store_model_samples = TRUE)
timer_regression.pl.end <- Sys.time()

traceplot.fit.regression.pl <- traceplot.modelSelection.regression.pl(fit.regression.pl, p)

plotcoda.fit.regression.pl <- plotcoda.modelSelection.regression.pl(fit.regression.pl, p)
...
plot(1:length(traceplot.fit.regression.pl), traceplot.fit.regression.pl, type = "l", col = "blue", xlab = "Iteration", ylab = "Graph size", main = "Trace of graph size")

plot(1:nrow(plotcoda.fit.regression.pl), plotcoda.fit.regression.pl[,1], type = "l", col = "blue", xlab = "Iteration", ylab = "Posterior link probability", ylim = range(plotcoda.fit.regression.pl), main = "Trace of Posterior Probability of Link")
for(i in 2:100){
  points(1:nrow(plotcoda.fit.regression.pl), plotcoda.fit.regression.pl[,i], type = "l", col = "blue")
}
